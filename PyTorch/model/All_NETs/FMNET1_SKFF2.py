...
#         L_Retinexformer = self.activate(self.conv3x3(L_Retinexformer))
#         L_Retinexformer += self.conv1x1_final(residual)
#
#         return L_Retinexformer
class FB(nn.Module):
    def __init__(self, n_feat, o_feat, kernel_size, bias, act):
...
class HFMNET(nn.Module):
    def __init__(self,in_chn=3, wf=96):
        ...
        self.hfb5 = FB(n_feat=768, o_feat=384, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.up3 = PS_up(384, 192, 2)

        self.hfb6 = FB(n_feat=384, o_feat=192, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.up2 = PS_up(192, 96, 2)

        self.hfb7 = FB(n_feat=192, o_feat=96, kernel_size=3, bias=False, act=nn.LeakyReLU())
        #输入分支
    ...
    def forward(self, x):
        ...
        dx3 = self.hfb5( torch.cat([dx3, skip[2]], dim=1) )
        skx3 = self.conv_x3(dx3)
        dx2 = self.up3(dx3)

        dx2 = self.hfb6(torch.cat([dx2, skip[1]], dim=1))
        skx2 = self.conv_x2(dx2)
        dx1 = self.up2(dx2)

        skx1 = self.hfb7(torch.cat([dx1, skip[0]], dim=1))
