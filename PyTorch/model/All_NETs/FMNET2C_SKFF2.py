...
class FB(nn.Module):
    ...
    def forward(self, x):
        ...
        # L_Retinexformer = torch.cat([wavelet_path, identity_path], dim=1)
...
class HFMNET(nn.Module):
    def __init__(self,in_chn=3, wf=96):
        ...
        #编码
        self.hfb1 = FB(n_feat=96, o_feat=96, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.down1 = PS_down(96,96,2)

        self.hfb2 = FB(n_feat=192, o_feat=192, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.down2 = PS_down(192,192,2)

        self.hfb3 = FB(n_feat=288, o_feat=384, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.down3 = PS_down(384,384,2)

        self.hfb4 = FB(n_feat=480, o_feat=768, kernel_size=3, bias=False, act=nn.LeakyReLU())
        self.down4 = PS_down(768, 768, 2)
        #底部
        self.hfb5 = FB(n_feat=864, o_feat=1536, kernel_size=3, bias=False, act=nn.LeakyReLU())
        #解码
        self.up4 = PS_up(1536, 768, 2)
        self.hfb6 = FB(n_feat=1920, o_feat=768, kernel_size=3, bias=False, act=nn.LeakyReLU())

        self.up3 = PS_up(768,384,2)
        self.hfb7 = FB(n_feat=960, o_feat=384, kernel_size=3, bias=False, act=nn.LeakyReLU())

        self.up2 = PS_up(384, 192, 2)
        self.hfb8 = FB(n_feat=576, o_feat=192, kernel_size=3, bias=False, act=nn.LeakyReLU())

        self.up1 = PS_up(192, 96, 2)
        self.hfb9 = FB(n_feat=288, o_feat=96, kernel_size=3, bias=False, act=nn.LeakyReLU())
        #输入分支
        ...
        self.up_branch5 = nn.Upsample(scale_factor=16, mode='bilinear', align_corners=False)
    ...
    def forward(self, x):
        ...
        x5 = self.conv_first(x5)  #96 16 16
        #编码
        x1_out = self.hfb1(x1)
        skip.append(x1_out)
        ...
        skip.append(x2_out)
        skip_updown.append(self.up1(x2_out))  #0 2上
        x2_down = self.down2(x2_out)
        skip_updown.append(x2_down)           #1 2下

        x3_out = self.hfb3( torch.cat([x3, x2_down], dim=1) )
        skip.append(x3_out)
        skip_updown.append(self.up2(x3_out))  #2 3上
        x3_down = self.down3(x3_out)
        skip_updown.append( x3_down )         #3 3下

        x4_out = self.hfb4(torch.cat([x4, x3_down], dim=1))
        skip.append(x4_out)
        x4_down = self.down4(x4_out)

        #底部
        dx5 = self.hfb5(torch.cat([x5,x4_down], dim=1))
        skx5 = self.conv_x5(dx5)

        #解码
        dx4 = self.up4(dx5)
        dx4 = self.hfb6(torch.cat([dx4, skip[3], skip_updown[3]], dim=1))
        skx4 = self.conv_x4(dx4)

        dx3 = self.up3(dx4)
        dx3 = self.hfb7( torch.cat([dx3, skip[2],skip_updown[1]], dim=1) )
        skx3 = self.conv_x3(dx3)

        dx2 = self.up2(dx3)
        dx2 = self.hfb8(torch.cat([dx2, skip[1],skip_updown[2]], dim=1))
        skx2 = self.conv_x2(dx2)

        #print(dx2.shape)
        dx1 = self.up1(dx2)
        skx1 = self.hfb9(torch.cat([dx1, skip[0],skip_updown[0]], dim=1))

        skx5 = self.up_branch5(skx5)
        skx4 = self.up_branch4(skx4)
        skx3 = self.up_branch3(skx3)
        skx2 = self.up_branch2(skx2)
        skx1 = self.conv_x1(skx1)

        last_out = self.skff([skx1,skx2,skx3,skx4,skx5])
        #last_out = skx1+skx2+skx3+skx4+skx5
...
if __name__=='__main__':
    ...
    print('parameters:', params / 1e6)  #56M
    print('flops', flops / 1e9)         #252
